...
def init(long N, long kA, long kP, long TR):
    #  fx size   oo.N+nfo1
    #  called like _kfar.init(oo.N, oo.k, oo.TR)
    #  (oo.N + 1) + 2    smpx x is this size
    global __N, _Np1, _kA, _TR, _kkA, kinfoA, p_kinfoA, kinfoP, p_kinfoP
    _TR = TR
    __N  =  N    #  this is oo.N
    _Np1= N+1
    _kA  = kA
    _kkA = kA*kA
    kinfoA = _N.zeros(3, dtype=_N.int32)
    cdef int[::1] v_kinfoA = kinfoA
    p_kinfoA     = &v_kinfoA[0]
    kinfoA[0] = kA
    kinfoA[1] = kA
    ...
    _kP  = kP
    _kkP = kP*kP
    kinfoP = _N.zeros(3, dtype=_N.int32)
    cdef int[::1] v_kinfoP = kinfoP
    p_kinfoP     = &v_kinfoP[0]
    kinfoP[0] = kP
    kinfoP[1] = kP
