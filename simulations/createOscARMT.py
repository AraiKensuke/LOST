...
def create(outdir):
    # _plt.ioff()
    global dt, lambda2, rpsth, isis, us, csTR, etme, bGenOscUsingAR, f0VAR, f0, Bf, Ba, amp, amp_nz, dSA, dSF, psth, prbs, prbsWithHist, latst, arq2
    latst = _N.empty((TR, N))
    ...
    if bGenOscUsingAR:
        ...
        for n in range(nRhythms):
            rs_fs.append([modulus_fs[n], rs[n]])

        innovar           = arq2
        ARcoeffs          = _blds.ARcoeffsFromList(rs_fs, dt=0.001)
    ...
    for tr in range(TR):
        if bGenOscUsingAR:
            ...
            x, dN, latst[tr], prbs, fs, prbsNOsc = createDataPPl2(TR, N, dt, ARcoeffs, wgts, us[tr], lambda2=lambda2, p=1, nRhythms=nRhythms, cs=csTR[tr], etme=etme[tr], offset=psth, innovar=innovar)
            AR_components[tr] = x
        else:
